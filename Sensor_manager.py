...
class SensorManager:
    ...
    def insert_data(self, temperature, humidity, moisture, probe_temperature):
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO sensors (temp_c, humidity, moisture, probe_temperature) VALUES (?, ?, ?, ?)",
    ...
    def print_latest_data(self):
        ...
        probe_temperature = data.get(
            "probe_temperature", "Unable to get Probe Temperature"
        )

        print(
            f"Latest Data - Temperature: {temperature:.2f}°C, Humidity: {humidity:.2f}%, Moisture: {moisture:.2f}%, Probe Temperature: {probe_temperature:.2f}°C"
        )
